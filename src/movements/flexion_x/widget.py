"""FlexionX movement widget for embedding in the main window."""
...
from . import original as flexion_x_original
from . import hybrid as flexion_x_hybrid
from . import force as flexion_x_force
...
METHOD_MODULES = {
    "original": flexion_x_original,
    "hybrid": flexion_x_hybrid,
    "force": flexion_x_force,
}



class FlexionXWidget(QWidget):
    """Widget containing flexion-x movement controls."""
    ...
    def __init__(self, app):
        ...
        info_label = QLabel("Rotates TCP around its own x-axis while maintaining zero force in TCP z-direction.")
        ...
        # Flexion Movement Parameters (expanded by default)
        self.movement_group = CollapsibleGroupBox("Flexion Movement Parameters", expanded=False)
        ...
        self.angle_input.setValue(CONFIG.flexion_x.angle)
        ...
        self.speed_input.setValue(CONFIG.flexion_x.speed)
        ...
        self.accel_input.setValue(CONFIG.flexion_x.acceleration)
        ...
        # Max TCP Fy (common to all methods)
        ...
        self.fy_limit_input.setMaximumWidth(80)
        self.fy_limit_input.setToolTip(TT["force_limit"])
        fy_layout.addWidget(self.fy_limit_input)
        self.movement_group.addLayout(fy_layout)
        
        # Max Moment Mx (common to all methods) - directional stopping criteria
        flexion_cfg = getattr(CONFIG, 'flexion_x', None)
        arc_cfg = getattr(CONFIG, 'arc_x', CONFIG.arc_force)
        max_moment_default = getattr(flexion_cfg, 'max_moment', None) or getattr(arc_cfg, 'max_moment', 3.0)
        mx_layout = QHBoxLayout()
        mx_layout.addWidget(QLabel("Max Moment Mx (Nm):"))
        self.mx_limit_input = QDoubleSpinBox()
        self.mx_limit_input.setRange(0.1, 50.0)
        self.mx_limit_input.setDecimals(2)
        self.mx_limit_input.setValue(max_moment_default)
        self.mx_limit_input.setSingleStep(0.1)
        self.mx_limit_input.setMaximumWidth(80)
        self.mx_limit_input.setToolTip(TT["arc_force_max_moment"])
        mx_layout.addWidget(self.mx_limit_input)
        self.movement_group.addLayout(mx_layout)
        ...
        self.gain_input.setValue(CONFIG.flexion_x_original.force_control_gain)
        ...
        self.deadband_input.setValue(CONFIG.flexion_x_original.force_deadband)
        ...
        self.max_adj_input.setValue(CONFIG.flexion_x_original.max_adjustment_per_step)
        ...
        self.interval_input.setValue(CONFIG.flexion_x_original.min_adjustment_interval)
        ...
        self.hybrid_z_limit_input.setValue(CONFIG.flexion_x_hybrid.force_mode_z_limit)
        ...
        self.hybrid_damping_input.setValue(CONFIG.flexion_x_hybrid.force_mode_damping)
        ...
        self.hybrid_params_group.setVisible(False)  # Hidden by default (Original selected)
        ...
        self.force_z_limit_input.setValue(CONFIG.flexion_x_force.force_mode_z_limit)
        ...
        self.force_damping_input.setValue(CONFIG.flexion_x_force.force_mode_damping)
        ...
        self.force_gain_scaling_input.setValue(CONFIG.flexion_x_force.force_mode_gain_scaling)
        ...
        self.control_dt_input.setValue(CONFIG.flexion_x_force.control_loop_dt)
        ...
        self.rot_speed_factor_input.setValue(CONFIG.flexion_x_force.rotation_speed_factor)
    ...
    def _get_current_method_module(self):
        ...
        return METHOD_MODULES.get(method, flexion_x_original)
    ...
    def collectWaypoints(self):
        """Execute flexion-x movement and save path."""
        checked_button = self.direction_group.checkedButton()
        ...
        force_limit_y = self.fy_limit_input.value()
        ...
        if hasattr(self.app, '_disable_freedrive_for_movement'):
            self.app._disable_freedrive_for_movement("flexion-x movement")
        
        try:
            # Get current TCP pose
            start_position = list(self.app.robot.rtdeReceive.getActualTCPPose())
            print(f"\n=== Flexion-X Movement (Set Path) ===")
            print(f"Starting TCP pose: [{start_position[0]:.4f}, {start_position[1]:.4f}, {start_position[2]:.4f}]")
            
            # Calculate new pose
            new_pose = rotate_pose_around_tcp_x(start_position, angle_deg * direction_multiplier)
            new_pose[0] = start_position[0]
            
            print(f"Rotating TCP frame {angle_deg * direction_multiplier}° around TCP x-axis...")
            ...
            print(f"Error during flexion-x set path: {e}")
            import traceback
            traceback.print_exc()
            self.reset_button_style()
...
def rotate_pose_around_tcp_x(pose: List[float], angle_deg: float) -> List[float]:
    """
    Rotate the TCP frame around its own (local) x-axis while keeping position fixed.

    Args:
        pose: Current TCP pose [x, y, z, rx, ry, rz]
        angle_deg: Rotation angle in degrees (positive = counterclockwise about TCP x)
    ...
    # Local TCP x rotation
    angle_rad = math.radians(angle_deg)
    R_tcp_x = Rotation.from_euler('x', angle_rad, degrees=False).as_matrix()

    # Apply local rotation: R_new = R_current @ R_tcp_x
    R_new = R_current @ R_tcp_x
