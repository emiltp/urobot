"""Arc (z) movement widget — moveL around ref Z axis, Fx/Fy/Fz compliant, Mz limit only."""
...
from . import movel as arc_z_movel
from . import get_path_filename as arc_z_get_path
from . import path_exists as arc_z_path_exists
from config import defaults as CONFIG

DIRECTION_MAP = {"left": 1, "right": -1}


class ArcZWidget(QWidget):
    """Widget for Arc (z): moveL around ref Z axis, Fx/Fy/Fz compliant, Mz limit only."""
    ...
    def __init__(self, app):
        ...
        info_label = QLabel(
            "Arc (z): moveL orbit around ref Z axis. Fx/Fy/Fz compliant (Z+XY limits), Mz limit only."
        )
        ...
        # Direction selection (same labels as New (z))
        ...
        # Movement Parameters
        arc_z_cfg = getattr(CONFIG, 'arc_z', CONFIG.arc_force)
        ...
        self.angle_input = self._add_spin(
            ...
            arc_z_cfg.angle, 1, 5.0, TT["angle"]
        )
        ...
        self.speed_input = self._add_spin(
            ...
            arc_z_cfg.speed, 4, 0.01, TT["speed"]
        )
        self.accel_input = self._add_spin(
            self.movement_group, "Acceleration (m/s²):", 0.001, 2.0,
            arc_z_cfg.acceleration, 3, 0.01, TT["acceleration"]
        )
        self.max_moment_input = self._add_spin(
            self.movement_group, "Max TCP Mz (Nm):", 0.1, 50.0,
            arc_z_cfg.max_moment, 2, 0.1, TT["arc_force_max_moment"]
        )
        layout.addWidget(self.movement_group)

        # Collection Method Parameters (Z + XY submenus + waypoints, like New z)
        self.collection_group = CollapsibleGroupBox("Collection Method Parameters", expanded=False)

        self.z_limit_group = CollapsibleGroupBox("Z Limit", expanded=False)
        self.z_limit_input = self._add_spin(
            self.z_limit_group, "Force Mode Z Limit (m/s):", 0.01, 0.2,
            arc_z_cfg.force_mode_z_limit, 3, 0.01, TT["force_mode_z_limit"]
        )
        self.z_damping_input = self._add_spin(
            self.z_limit_group, "Damping:", 0.0, 1.0,
            arc_z_cfg.force_mode_damping, 2, 0.05, TT["force_mode_damping"]
        )
        self.z_gain_input = self._add_spin(
            self.z_limit_group, "Gain scaling:", 0.0, 2.0,
            arc_z_cfg.force_mode_gain_scaling, 2, 0.1, TT["force_mode_gain_scaling"]
        )
        self.collection_group.addWidget(self.z_limit_group)

        self.xy_limit_group = CollapsibleGroupBox("XY Plane Limits", expanded=False)
        self.xy_limit_input = self._add_spin(
            self.xy_limit_group, "Force Mode XY Limit (m/s):", 0.01, 0.2,
            getattr(arc_z_cfg, 'force_mode_xy_limit', arc_z_cfg.force_mode_z_limit), 3, 0.01, TT.get("force_mode_xy_limit", TT["force_mode_z_limit"])
        )
        self.xy_damping_input = self._add_spin(
            self.xy_limit_group, "Damping:", 0.0, 1.0,
            arc_z_cfg.force_mode_damping, 2, 0.05, TT["force_mode_damping"]
        )
        self.xy_gain_input = self._add_spin(
            self.xy_limit_group, "Gain scaling:", 0.0, 2.0,
            arc_z_cfg.force_mode_gain_scaling, 2, 0.1, TT["force_mode_gain_scaling"]
        )
        self.collection_group.addWidget(self.xy_limit_group)
        ...
        self.waypoint_count_input = self._add_spin(
            ...
            arc_z_cfg.waypoint_count, 0, 10, TT["waypoint_count"]
        )
        self.target_distance_input = self._add_spin(
            self.waypoint_group, "Target distance (m):", 0.0005, 0.01,
            arc_z_cfg.target_distance, 4, 0.001, TT["target_distance"]
        )
        ...
        self.replay_speed_input.setValue(arc_z_cfg.speed)
    ...
    def _update_button_states(self):
        ...
        if connected and has_direction:
            ...
            path_file_exists = arc_z_path_exists(direction)
            if path_file_exists:
    ...
    def _on_set_path_clicked(self):
        ...
        if hasattr(self.app, '_disable_freedrive_for_movement'):
            self.app._disable_freedrive_for_movement("arc z")

        direction = checked_button.direction
        path_file = arc_z_get_path(direction)
        ...
        try:
            ...
            print(f"\n=== Arc (z) — Set Path ===")
            print(f"moveL around ref Z, Fx/Fy/Fz compliant, Mz limit. Direction: {checked_button.direction}")
            ...
            method_kwargs = {
                ...
                'force_mode_xy_limit': self.xy_limit_input.value(),
                'force_mode_z_damping': self.z_damping_input.value(),
                'force_mode_xy_damping': self.xy_damping_input.value(),
                'force_mode_z_gain_scaling': self.z_gain_input.value(),
                'force_mode_xy_gain_scaling': self.xy_gain_input.value(),
            ...
            self.app.async_motion_runner.movement_started.connect(lambda: print("Starting Arc (z) Set Path..."))
            ...
            print(f"Error during Arc (z) Set Path: {e}")
            import traceback
            traceback.print_exc()
            self.reset_button_style()
    ...
    def _on_run_clicked(self):
        ...
        direction = checked_button.direction
        path_file = arc_z_get_path(direction)
        if not arc_z_path_exists(direction):
        ...
        if hasattr(self.app, '_disable_freedrive_for_movement'):
            self.app._disable_freedrive_for_movement("arc z traverse")
