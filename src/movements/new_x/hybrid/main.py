"""
New X Hybrid - Force mode wrapper around moveL.

Uses force mode to maintain Fz=0 while executing a standard moveL
for the orbital motion. The robot follows the planned trajectory with
Z-axis compliance.
"""
...
def execute(self):
    """Execute new-x movement using hybrid approach (force mode + moveL)."""
    ...
    forceModeZLimit = self.kwargs.get('force_mode_z_limit', CONFIG.new_x_hybrid.force_mode_z_limit)
    forceModeDamping = self.kwargs.get('force_mode_damping', CONFIG.new_x_hybrid.force_mode_damping)
    forceModeGainScaling = self.kwargs.get('force_mode_gain_scaling', CONFIG.new_x_hybrid.force_mode_gain_scaling)
    ...
    self.movement_progress.emit("Starting new-x movement (hybrid: force mode + moveL)...")
    ...
    try:
        ...
        while self.rtde_c.getAsyncOperationProgress() >= 0 and not self._stop_requested:
            ...
            if (currentTime - lastWaypointTime) >= waypointInterval:
                ...
                self.movement_progress.emit(
                    f"Progress: {progress} | Fy: {forceYTcp:.2f} N ({limitDisplay}) | "
        ...
        try:
            ...
            pass
        self.movement_progress.emit(f"Error during new-x hybrid movement: {str(e)}")
        raise
