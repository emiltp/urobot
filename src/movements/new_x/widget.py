"""NewX movement widget for embedding in the main window."""
...
from . import original as new_x_original
from . import hybrid as new_x_hybrid
from . import force as new_x_force
...
class NewXWidget(QWidget):
    """Widget containing new-x movement controls."""
    ...
    def __init__(self, app):
        ...
        info_label = QLabel("New (x) motion — placeholder description.")
        ...
        self.angle_input.setValue(CONFIG.new_x.angle)
        ...
        self.speed_input.setValue(CONFIG.new_x.speed)
        ...
        self.accel_input.setValue(CONFIG.new_x.acceleration)
        ...
        self.fy_limit_input.setValue(CONFIG.new_x.force_limit_y)
        ...
        self.gain_input.setValue(CONFIG.new_x_original.force_control_gain)
        ...
        self.deadband_input.setValue(CONFIG.new_x_original.force_deadband)
        ...
        self.max_adj_input.setValue(CONFIG.new_x_original.max_adjustment_per_step)
        ...
        self.interval_input.setValue(CONFIG.new_x_original.min_adjustment_interval)
        ...
        self.hybrid_z_limit_input.setValue(CONFIG.new_x_hybrid.force_mode_z_limit)
        ...
        self.hybrid_damping_input.setValue(CONFIG.new_x_hybrid.force_mode_damping)
        ...
        self.force_z_limit_input.setValue(CONFIG.new_x_force.force_mode_z_limit)
        ...
        self.force_damping_input.setValue(CONFIG.new_x_force.force_mode_damping)
        ...
        self.force_gain_scaling_input.setValue(CONFIG.new_x_force.force_mode_gain_scaling)
        ...
        self.control_dt_input.setValue(CONFIG.new_x_force.control_loop_dt)
        ...
        self.rot_speed_factor_input.setValue(CONFIG.new_x_force.rotation_speed_factor)
        ...
        self.replay_blend_input.setToolTip("Blend radius for smooth waypoint transitions (0 = sharp corners)")
    ...
    def _get_current_method_module(self):
        ...
        return METHOD_MODULES.get(method, new_x_original)
    ...
    def _on_angle_changed(self, value):
        """Re-compute endpoint visualization when rotation angle changes."""
    ...
    def _update_endpoint_visualization(self):
        """Compute the TCP endpoint orbited around the Ref frame's local x-axis.

        The endpoint keeps the same distance to the Ref frame origin as the
        current TCP, but is rotated around the Ref frame's x-axis by the
        specified angle and direction.
        """
        checked_button = self.direction_group.checkedButton()
        ...
        endpoint_pose = _orbit_tcp_around_ref_x(tcp_pose, ref_pose, angle_rad)
    ...
    def _remove_endpoint_visualization(self):
        """Remove the endpoint axes actor from the scene."""
    ...
    def collectWaypoints(self):
        """Execute new-x movement and save path."""
        checked_button = self.direction_group.checkedButton()
        ...
        if hasattr(self.app, '_disable_freedrive_for_movement'):
            self.app._disable_freedrive_for_movement("new-x movement")
        
        try:
            start_position = list(self.app.robot.rtdeReceive.getActualTCPPose())
            print(f"\n=== New-X Movement (Set Path) ===")
            ...
            new_pose = _orbit_tcp_around_ref_x(start_position, ref_pose, angle_rad)
            
            print(f"Orbiting TCP {angle_deg * direction_multiplier}° around Ref frame x-axis...")
            ...
            print(f"Error during new-x set path: {e}")
            import traceback
            traceback.print_exc()
            self.reset_button_style()
...
def _orbit_tcp_around_ref_x(tcp_pose: List[float], ref_pose: List[float],
                             angle_rad: float) -> List[float]:
    """Orbit the TCP around the Ref frame's local x-axis.
    ...
    local x-axis.  The TCP orientation is rotated by the same amount.

    Args:
        tcp_pose: Current TCP pose [x, y, z, rx, ry, rz] in base frame.
        ref_pose: Ref frame pose [x, y, z, rx, ry, rz] in base frame.
        angle_rad: Rotation angle in radians around the Ref frame x-axis.
    ...
    # Vector from ref origin to TCP, expressed in ref-local coordinates
    offset_base = tcp_pos - ref_pos
    offset_local = R_ref.T @ offset_base

    # Rotate that local vector around the ref-local x-axis
    R_x = Rotation.from_rotvec([angle_rad, 0, 0]).as_matrix()
    offset_local_rotated = R_x @ offset_local

    # Back to base frame
    new_pos = ref_pos + R_ref @ offset_local_rotated

    # Rotate the TCP orientation by the same rotation (applied in base frame)
    R_base_rotation = R_ref @ R_x @ R_ref.T
