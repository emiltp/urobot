"""Arc (x) movement widget — moveL around ref X axis, Fz=0, Mx/Fy limits, null sensing."""
...
from src.movements.arc_force import movel as arc_movel
from . import get_path_filename as arc_x_get_path
from . import path_exists as arc_x_path_exists
from config import defaults as CONFIG

DIRECTION_MAP = {"left": -1, "right": 1}


class ArcXWidget(QWidget):
    """Widget for Arc (x): moveL around ref X axis, Fz=0, Mx and Fy limits, null sensing."""
    ...
    def __init__(self, app):
        ...
        info_label = QLabel(
            "Arc (x): moveL orbit around ref X axis. Fz=0, Mx and Fy limits, null sensing."
        )
    ...
    def _update_button_states(self):
        ...
        if connected and has_direction:
            ...
            path_file_exists = arc_x_path_exists(direction)
            if path_file_exists:
    ...
    def _on_set_path_clicked(self):
        ...
        if hasattr(self.app, '_disable_freedrive_for_movement'):
            self.app._disable_freedrive_for_movement("arc x")

        direction = checked_button.direction
        path_file = arc_x_get_path(direction)
        arc_cfg = getattr(CONFIG, 'arc_x', CONFIG.arc_force)
        ...
        try:
            ...
            print(f"\n=== Arc (x) — Set Path ===")
            print(f"moveL around ref X, null sensing. Direction: {checked_button.direction}")
            ...
            self.app.async_motion_runner.movement_started.connect(lambda: print("Starting Arc (x) Set Path..."))
            ...
            print(f"Error during Arc (x) Set Path: {e}")
            import traceback
            traceback.print_exc()
            self.reset_button_style()
    ...
    def _on_run_clicked(self):
        ...
        direction = checked_button.direction
        path_file = arc_x_get_path(direction)
        if not arc_x_path_exists(direction):
        ...
        if hasattr(self.app, '_disable_freedrive_for_movement'):
            self.app._disable_freedrive_for_movement("arc x traverse")
