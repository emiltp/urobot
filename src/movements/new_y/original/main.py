"""
New Y Original - Software-based Fz compensation with stop-adjust-restart.

Asynchronous moveL to the orbital target with active Fz software compensation.
The TCP orbits around the Ref frame's y-axis, so all three position axes may
change and force adjustments use the full TCP Z direction.
"""
...
def execute(self):
    """Execute new-y movement (original method)."""
    ...
    forceControlGain = self.kwargs.get('force_control_gain', CONFIG.new_y_original.force_control_gain)
    forceDeadband = self.kwargs.get('force_deadband', CONFIG.new_y_original.force_deadband)
    maxAdjustmentPerStep = self.kwargs.get('max_adjustment_per_step', CONFIG.new_y_original.max_adjustment_per_step)
    minAdjustmentInterval = self.kwargs.get('min_adjustment_interval', CONFIG.new_y_original.min_adjustment_interval)
    ...
    self.movement_progress.emit("Starting new-y movement...")
    ...
    try:
        ...
        while self.rtde_c.getAsyncOperationProgress() >= 0 and not self._stop_requested:
            ...
            self.movement_progress.emit(
                ...
                f"  Fx: {forceXTcp:.3f} N ({limitDisplay})\n"
        ...
        self.movement_progress.emit(f"Error during new-y movement: {str(e)}")
        raise
